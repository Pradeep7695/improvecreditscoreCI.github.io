						...
						<table id="testimonial" class="table table-bordered table-striped table-actions">
							...
							<tbody>
								<?php if ($testimonialData):?>
								   <?php foreach ($testimonialData as $testimonialData):?>
									<tr>
										<td><?= $testimonialData->id?></td>
										<td><?= $testimonialData->client_name;?></td>
										<td><?= $testimonialData->client_review;?></td>
										<td><img src="<?= $testimonialData->client_img;?>" class="img-responsive img-thumbnail" width="100" height="100"></td>
										<td><?= date('d-m-Y',strtotime($testimonialData->created_at))?></td>
										<td>
											<?php
											echo anchor("dashboard/edit_testimonial/". $testimonialData->id,'<button class="btn btn-info btn-condensed"><i class="fa fa-edit"></i></button>');
										    echo anchor("dashboard/delete_testimonial/". $testimonialData->id,'<button class="btn btn-danger btn-condensed" onclick="return confirm(\'Are you sure you want to delete this ?\');"><i class="fa fa-trash"></i></button>')
											?>
										</td>
									</tr>
								   <?php endforeach;?>
